...
export default function CommercialPhotos() {
    ...
    async function fetchPhotos() {
      ...
      if (!error && data) {
        const items = data
          ...
          .map((f) => {
            const { data: urlData } = supabase.storage.from(BUCKETS.commercialPhotos).getPublicUrl(f.name);
      ...
      <div className="relative w-full h-[90vh] flex justify-center items-center py-24 px-4 md:px-10 mb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center text-gray-200 space-y-6">
          ...
          <p>
            ...
            from product shots to corporate headshots, crafted to make an impression.
          </p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 text-white/30">
            ...
            <p className="text-sm mt-2">
              Upload to the <span className="text-[#C8A96A]">commercial-photos</span> bucket in Admin.
            </p>
          </div>
        ) : (
          <div className="[column-count:2] md:[column-count:3] lg:[column-count:4] [column-gap:1rem]">
            {images.map((img, index) => (
              <div key={img.src} className="relative mb-4 overflow-hidden rounded-xl group break-inside-avoid">
                <img src={img.src} alt={`commercial-${index}`}
