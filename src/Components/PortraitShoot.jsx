...
export default function PortraitShoot() {
    ...
    async function fetchPhotos() {
      ...
      if (!error && data) {
        const items = data
          ...
          .map((f) => {
            const { data: urlData } = supabase.storage.from(BUCKETS.portraitShoot).getPublicUrl(f.name);
      ...
      <div className="relative w-full h-[90vh] flex justify-center items-center py-24 px-4 md:px-10 mb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center text-gray-200 space-y-6">
          ...
          <p>
            ...
            space where your personality shines through naturally.
          </p>
        </div>
      </div>
      ...
      <div className="relative overflow-hidden px-4 md:px-10 pb-16">
        ...
        {loading ? (
          ...
          <div className="relative z-10 text-center py-24 text-white/30">
            ...
            <p className="text-sm mt-2">Upload to the <span className="text-[#C8A45D]">portrait-shoot</span> bucket in Admin.</p>
          </div>
        ) : (
