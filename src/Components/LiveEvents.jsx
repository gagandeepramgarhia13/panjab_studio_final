        ...
        async function fetchPhotos() {
            ...
            if (!error && data) {
                const items = data
                    ...
                    .map((f) => {
                        const { data: urlData } = supabase.storage.from(BUCKETS.liveEvents).getPublicUrl(f.name);
        ...
        <section className="relative w-full min-h-screen z-0">
            ...
            <div className="relative w-full h-[90vh] flex justify-center items-center py-24 px-4 md:px-10 mb-16 overflow-hidden">
                <img
                    ...
                    src="https://i.pinimg.com/1200x/6e/3e/4e/6e3e4e9e8a1b2c3d4e5f6a7b8c9d0e1f.jpg"
                    alt=""
                ...
                <div className="relative z-10 max-w-4xl mx-auto text-center text-gray-200 space-y-6">
                    ...
                    <p>
                        ...
                        atmosphere of every live event — frame by frame.
                    </p>
                </div>
            </div>
            ...
            <div className="px-4 md:px-10 pb-16">
                {loading ? (
                    ...
                    <div className="text-center py-24 text-white/30">
                        ...
                        <p className="text-sm mt-2">Upload to the <span className="text-[#C8A96A]">live-events</span> bucket in Admin.</p>
                    </div>
                ) : (
                    <div className="[column-count:2] md:[column-count:3] lg:[column-count:4] [column-gap:1rem]">
                        {images.map((img, index) => (
                            <div key={img.src} className="relative mb-4 overflow-hidden rounded-xl group break-inside-avoid">
                                <img src={img.src} alt={`live-event-${index}`} className="w-full h-auto block transition duration-500 group-hover:scale-105" />
