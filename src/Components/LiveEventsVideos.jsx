  ...
  useEffect(() => {
    async function fetchVideos() {
      const { data, error } = await supabase.storage
        .from(BUCKETS.liveEventsVideos)
        .list("", { limit: 200, sortBy: { column: "created_at", order: "desc" } });
      if (!error && data) {
        const urls = data
          .filter(f => f.name !== ".emptyFolderPlaceholder")
          .map(f => {
            const { data: urlData } = supabase.storage.from(BUCKETS.liveEventsVideos).getPublicUrl(f.name);
      ...
      <div className="relative overflow-hidden px-4 md:px-10 py-16">
        ...
        {loading ? (
          ...
          <div className="relative z-10 text-center py-24 text-white/30">
            ...
            <p className="text-sm mt-2">Upload to the <span className="text-[#C8A96A]">live-events-videos</span> bucket in Admin.</p>
          </div>
        ) : (
