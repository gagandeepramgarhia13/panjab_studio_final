    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div key={t.id} className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium shadow-2xl backdrop-blur-xl border
          ${t.type === "success" ? "bg-[#C8A96A]/90 text-black border-[#C8A96A]/50" : "bg-red-500/90 text-white border-red-400/50"}`}>
          {t.type === "success" ? <CheckCircle size={16} /> : <AlertCircle size={16} />}
          {t.message}
        </div>
      ))}
  ...
  return (
    <div
      ...
      className={`relative flex flex-col items-center justify-center gap-3 border-2 border-dashed
        ...
        ${uploading ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}
        ${dragging ? "border-[#C8A96A] bg-[#C8A96A]/10 scale-[1.01]"
          : "border-white/20 bg-white/5 hover:border-[#C8A96A]/60 hover:bg-white/10"}`}
    >
      <input ref={inputRef} type="file" accept={accept} multiple className="hidden"
        onChange={(e) => handle(e.target.files)} />
      <div className="w-16 h-16 rounded-full bg-[#C8A96A]/15 backdrop-blur-xl border border-[#C8A96A]/30 flex items-center justify-center">
        {uploading
          ? <div className="w-7 h-7 border-2 border-[#C8A96A] border-t-transparent rounded-full animate-spin" />
          : <Icon size={28} className="text-[#C8A96A]" />}
      </div>
      ...
      <span className="text-xs text-[#C8A96A]/80 bg-[#C8A96A]/10 border border-[#C8A96A]/20 px-3 py-1 rounded-full">
        {uploading ? "Uploading…" : accept.split(",").join(" · ")}
      </span>
    </div>
  );
...
function MediaCard({ item, onDelete, onPreview }) {
  return (
    <div className="group relative rounded-2xl overflow-hidden bg-white/10 backdrop-blur-xl border border-white/20 hover:border-[#C8A96A]/50 transition-all duration-300 shadow-lg">
      <div className="aspect-square relative">
        {item.type === "photo"
          ? <img src={item.url} alt={item.name} className="w-full h-full object-cover" />
...
function MessageCard({ msg, onMarkRead, onDelete }) {
  return (
    <div className={`relative rounded-2xl border backdrop-blur-xl p-5 transition-all duration-300 shadow-lg
      ${msg.is_read ? "bg-white/10 border-white/20" : "bg-[#C8A96A]/15 border-[#C8A96A]/40"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          {!msg.is_read && <span className="w-2 h-2 rounded-full bg-[#C8A96A]" />}
          <p className="text-white font-semibold text-sm flex items-center gap-2">
...
function StatCard({ label, value, icon: Icon }) {
  return (
    <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-5 flex items-center gap-4 shadow-lg">
      <div className="w-12 h-12 rounded-xl bg-[#C8A96A] flex items-center justify-center shadow-md">
        <Icon size={22} className="text-black" />
      </div>
      <div>
